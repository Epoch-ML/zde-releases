...
async function writeRawTar(archivePath, entries) {
  ...
  for (const entry of entries) {
    ...
    const header = new Header({
      ...
      path: entry.path.length > 100 ? "ZDE.app/pax-placeholder" : entry.path,
      size,
...
async function makeApplication() {
  const root = await mkdtemp(join(tmpdir(), "zde-macos-package-"));
  temporaryDirectories.push(root);
  const applicationPath = join(root, "bundle", "ZDE.app");
  const executablePath = join(applicationPath, "Contents", "MacOS", "ZDE");
...
describe("deterministic macOS updater packaging", () => {
  it("produces identical archives with a complete sorted app tree", async () => {
    ...
    assert.deepEqual(names, [
      "ZDE.app/",
      "ZDE.app/Contents/",
      "ZDE.app/Contents/MacOS/",
      "ZDE.app/Contents/MacOS/ZDE",
      "ZDE.app/Contents/Resources/",
      "ZDE.app/Contents/Resources/version.txt",
    ]);
    assert.deepEqual(
      [...new Set(mtimes)],
  ...
  it("extracts one bounded regular app tree without executing payload files", async () => {
    const fixture = await makeApplication();
    await rm(join(fixture.applicationPath, "Contents", "version-link"));
    const archivePath = join(fixture.root, "ZDE.source.app.tar.gz");
    ...
    assert.equal(result.applicationPath, join(extractionRoot, "ZDE.app"));
    assert.equal(
      await readFile(join(result.applicationPath, "Contents", "MacOS", "ZDE"), "utf8"),
      "#!/bin/sh\nexit 0\n",
    );
  ...
  it("rejects every non-contained archive member path and a missing explicit root", async () => {
    ...
    await assert.rejects(
      ...
      /archive must contain exactly one ZDE\.app root/,
    );

    const longPathArchive = join(fixture.root, "long-path.app.tar.gz");
    const longPath = "ZDE.app/" + "a".repeat(4_089);
  ...
  it("rejects duplicate archive members", async () => {
    ...
    await writeRawTar(duplicateArchive, [
      { path: "ZDE.app/", type: "Directory" },
      { contents: "first", path: "ZDE.app/value" },
      { contents: "second", path: "ZDE.app/value" },
    ]);
    await assert.rejects(
      extractSourceApplication({
      ...
      /archive contains a duplicate path: ZDE\.app\/value/,
    );

    const fileRootArchive = join(fixture.root, "file-root.app.tar.gz");
    await writeRawTar(fileRootArchive, [
      { contents: "not an application directory", path: "ZDE.app" },
    ]);
    await assert.rejects(
      extractSourceApplication({
      ...
      /archive ZDE\.app root must be a directory/,
    );
  ...
  it("keeps extraction outside the archive and removes a partially extracted tree", async () => {
    ...
    await writeRawTar(conflictingArchive, [
      { path: "ZDE.app/", type: "Directory" },
      { contents: "file", path: "ZDE.app/parent" },
      { contents: "child", path: "ZDE.app/parent/child" },
    ]);
    ...
    await assert.rejects(
      ...
      /archive path hierarchy conflicts at ZDE\.app\/parent/,
    );
