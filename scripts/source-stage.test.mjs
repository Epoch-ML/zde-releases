...
async function makeFixture(metadataOverrides = {}) {
  const root = await mkdtemp(join(tmpdir(), "zde-source-stage-"));
  temporaryDirectories.push(root);
  const applicationPath = join(root, "application", "ZDE.app");
  ...
  const executable = join(applicationPath, "Contents", "MacOS", "ZDE");
  ...
  const archiveName = "ZDE_0.2.0-preview.1_aarch64.source.app.tar.gz";
...
describe("hostile ZDE source stage", () => {
  it("binds one unsigned app archive to the immutable request and channel key", async () => {
    ...
    assert.equal(result.applicationPath, join(fixture.outputDirectory, "ZDE.app"));
    assert.equal(
  ...
  it("rejects every source-stage metadata field that is not request-bound", async () => {
    const cases = [
      ...
      [{ release_tag: "zde-preview-v0.2.1" }, /source-stage release tag does not match/],
