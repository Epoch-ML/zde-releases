...
async function makeFixture(overrides = {}) {
  const root = await mkdtemp(join(tmpdir(), "zde-signed-release-"));
  temporaryDirectories.push(root);
  const inputDirectory = join(root, "input");
  const outputDirectory = join(root, "output");
  const publicKeyPath = join(root, "trusted.pubkey");
  await mkdir(inputDirectory, { recursive: true });
  await writeFile(join(inputDirectory, "ZDE.app.tar.gz"), "signed app archive");
  await writeFile(join(inputDirectory, "ZDE.app.tar.gz.sig"), "encoded updater signature\n");
  await writeFile(join(inputDirectory, "ZDE_0.2.0_aarch64.dmg"), "signed disk image");
...
describe("trusted ZDE updater release collection", () => {
    ...
    assert.deepEqual(result.manifest, {
      ...
      platforms: {
        "darwin-aarch64": {
          ...
          url: "https://github.com/Epoch-ML/zde-releases/releases/download/zde-preview-v0.2.0-preview.1/ZDE.app.tar.gz",
        },
      },
    });
  ...
  it("percent-encodes preview build metadata in immutable release URLs", async () => {
    const fixture = await makeFixture({
      ...
      release_tag: "zde-preview-v0.2.0-preview.1+arm64",
    });
    const result = await collectSignedRelease({
      ...fixture,
      releaseRepository: "Epoch-ML/zde-releases",
      request: previewRequest({
        version: "0.2.0-preview.1+arm64",
        releaseTag: "zde-preview-v0.2.0-preview.1+arm64",
      }),
    });
    ...
    assert.equal(
      ...
      "https://github.com/Epoch-ML/zde-releases/releases/download/" +
        "zde-preview-v0.2.0-preview.1%2Barm64/ZDE.app.tar.gz",
    );
  });
  ...
  it("requires notarization for stable and exactly one non-empty signed artifact set", async () => {
    ...
    await writeFile(join(emptySignature.inputDirectory, "ZDE.app.tar.gz.sig"), "  \n");
    await assert.rejects(
      collectSignedRelease({
        ...emptySignature,
        releaseRepository: "Epoch-ML/zde-releases",
    ...
    await writeFile(join(ambiguous.inputDirectory, "ZDE-copy.app.tar.gz"), "other archive");
    await assert.rejects(
      collectSignedRelease({
        ...ambiguous,
        releaseRepository: "Epoch-ML/zde-releases",
