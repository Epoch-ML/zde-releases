...
const releaseTag = `zde-preview-v${version}`;
const releaseRepository = "Epoch-ML/zde-releases";
...
async function makePayload(label, payloadRequest = request()) {
  const root = await mkdtemp(join(tmpdir(), "zde-release-payload-"));
  temporaryDirectories.push(root);
  const directory = join(root, "release");
  await mkdir(directory);
  const archiveName = `ZDE_${payloadRequest.version}_aarch64.app.tar.gz`;
  const signatureName = `${archiveName}.sig`;
  const diskImageName = `ZDE_${payloadRequest.version}_aarch64.dmg`;
...
describe("canonical ZDE release payload verification", () => {
  it("verifies exact artifact integrity and request provenance through API and CLI", async () => {
    ...
    assert.deepEqual(result.assetNames, [
      `ZDE_${version}_aarch64.app.tar.gz`,
      `ZDE_${version}_aarch64.app.tar.gz.sig`,
      `ZDE_${version}_aarch64.dmg`,
      "checksums.txt",
      "latest.json",
  ...
  it("validates option, repository, request, and directory boundaries", async () => {
    ...
    for (const invalidRepository of [
      "!Epoch-ML/zde-releases",
      "Epoch-ML/zde-releases!",
      "Epoch-ML/zde-releases/extra",
    ]) {
      await expectPayloadError(
        { ...valid, releaseRepository: invalidRepository },
  ...
  it("validates every release-metadata trust and integrity field", async () => {
    ...
    const stableRequest = {
      ...
      sourceRef: "refs/tags/zde-v0.2.0",
      version: "0.2.0",
    };
    ...
    assert.equal(stableResult.archiveName, "ZDE_0.2.0_aarch64.app.tar.gz");
  ...
  it("validates every updater-manifest provenance, platform, signature, and URL field", async () => {
    ...
    await rewriteJson(wrongUrl.directory, "latest.json", (manifest) => {
      manifest.platforms["darwin-aarch64"].url =
        "https://github.com/Epoch-ML/zde-releases/releases/download/other/file";
    });
    await expectPayloadError(
      {
