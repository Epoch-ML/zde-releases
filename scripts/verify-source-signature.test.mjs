...
const failedArtifactDetails = `Executable=/private/tmp/ZDE.app/Contents/MacOS/app
...
async function makeTemporaryDirectory() {
  const directory = await mkdtemp(join(tmpdir(), "zde-source-signature-"));
  temporaryDirectories.push(directory);
  return directory;
}

describe("ZDE source signature policy", () => {
  ...
  it("accepts only codesign's explicit unsigned failure", () => {
    assert.deepEqual(
      verifySourceSignature(
        "/private/tmp/ZDE.app: code object is not signed at all\n",
        255,
      ),
      { kind: "unsigned" },
    );
