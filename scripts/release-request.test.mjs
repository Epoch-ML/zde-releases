...
function makeRequest(overrides = {}) {
  return {
    ...
    release_tag: "zde-preview-v0.2.0-preview.1",
    source_repository: "Epoch-ML/zerg",
    source_sha: sourceSha,
    source_ref: "refs/tags/zde-preview-v0.2.0-preview.1",
...
describe("ZDE release request validation", () => {
  it("accepts one canonical preview request and derives immutable build metadata", () => {
    const result = validateReleaseRequest(makeRequest(), {
      requestFilename: "zde-preview-v0.2.0-preview.1.json",
    });

    assert.deepEqual(result, {
      channel: "preview",
      requestedAt,
      releaseTag: "zde-preview-v0.2.0-preview.1",
      sourceRef: "refs/tags/zde-preview-v0.2.0-preview.1",
  ...
  it("accepts numeric stable versions and rejects prerelease stable versions", () => {
    const stable = makeRequest({
      channel: "stable",
      version: "12.34.56",
      release_tag: "zde-v12.34.56",
      source_ref: "refs/tags/zde-v12.34.56",
    });
    assert.equal(
      validateReleaseRequest(stable, { requestFilename: "zde-v12.34.56.json" }).channel,
      "stable",
    );
    ...
    assert.throws(
      () => validateReleaseRequest({
        ...
        release_tag: "zde-v12.34.56-rc.1",
        source_ref: "refs/tags/zde-v12.34.56-rc.1",
      }, { requestFilename: "zde-v12.34.56-rc.1.json" }),
    ...
    assert.throws(
      () => validateReleaseRequest({
        ...
        release_tag: "zde-v12.34.56+rebuilt",
        source_ref: "refs/tags/zde-v12.34.56+rebuilt",
      }),
  ...
  it("enforces anchored SemVer and SHA boundaries while accepting full identifiers", () => {
    ...
    for (const version of invalidVersions) {
      assert.throws(
        () => validateReleaseRequest(makeRequest({
          version,
          release_tag: "zde-preview-v" + version,
          source_ref: "refs/tags/zde-preview-v" + version,
        })),
    ...
    const full = validateReleaseRequest(makeRequest({
      version: fullVersion,
      release_tag: "zde-preview-v" + fullVersion,
      source_ref: "refs/tags/zde-preview-v" + fullVersion,
    }));
    ...
    assert.equal(validateReleaseRequest(makeRequest({
      ...
      release_tag: "zde-preview-v" + oneCharacterBuild,
      source_ref: "refs/tags/zde-preview-v" + oneCharacterBuild,
    })).version, oneCharacterBuild);
  ...
  it("rejects schema drift, malformed provenance, and a mismatched filename", () => {
    assert.throws(
      () => validateReleaseRequest(makeRequest({
        release_tag: "zde-preview-v0.2.0-preview.2",
      })),
      /release tag must be zde-preview-v0.2.0-preview.1/,
    );
    assert.throws(
    ...
    assert.throws(
      ...
      /request filename must be zde-preview-v0.2.0-preview.1.json/,
    );
  });

  it("rejects another product, repository, channel, ref, or non-canonical timestamp", () => {
    const invalidRequests = [
      [makeRequest({ product: "Other" }), /product must be ZDE/],
      ...
      [makeRequest({ source_ref: "refs/heads/main" }), /source ref must be refs\/tags\/zde-preview-v0.2.0-preview.1/],
  ...
  it("parses a request file while rejecting malformed JSON", async () => {
    const directory = await mkdtemp(join(tmpdir(), "zde-public-request-test-"));
    temporaryDirectories.push(directory);
    const validPath = join(directory, "zde-preview-v0.2.0-preview.1.json");
    const invalidPath = join(directory, "zde-preview-v0.2.0-preview.2.json");
    const wrongNamePath = join(directory, "wrong.json");
    const symlinkPath = join(directory, "zde-preview-v0.2.0-preview.3.json");
    ...
    assert.equal(request.releaseTag, "zde-preview-v0.2.0-preview.1");
  ...
  it("emits every validated CLI output and fails without an input path", async () => {
    const directory = await mkdtemp(join(tmpdir(), "zde-request-cli-test-"));
    temporaryDirectories.push(directory);
    const requestPath = join(directory, "zde-preview-v0.2.0-preview.1.json");
    ...
    assert.equal(JSON.parse(withoutOutput.stdout).releaseTag, "zde-preview-v0.2.0-preview.1");
