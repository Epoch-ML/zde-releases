...
describe("ZDE release workflow contract", () => {
  ...
  it("recovers published immutable retries from canonical public bytes", () => {
    ...
    assert.match(publishJob, /name: zde-v2-canonical-release/);
    assert.match(feedJob, /name: zde-v2-canonical-release/);
    assert.match(pagesJob, /name: zde-v2-canonical-release/);
    assert.doesNotMatch(
      feedJob,
      /name: zde-v2-macos/,
    ...
    const canonicalDownload = publishJob.slice(
      ...
      publishJob.indexOf("name: zde-v2-canonical-release"),
    );
    assert.match(
      canonicalDownload,
