...
async function makeTemporaryDirectory() {
  const directory = await mkdtemp(join(tmpdir(), "zde-release-resolution-"));
...
describe("GitHub draft release resolution", () => {
  it("selects one exact draft tag across paginated results", () => {
    ...
    const pages = [
      ...
      [draft, { id: 2, tag_name: "zde-v0.1.0", draft: false }],
    ];

    assert.deepEqual(resolveReleaseByTag(pages, draft.tag_name), draft);
    assert.equal(resolveReleaseByTag(pages, "zde-preview-v9.9.9"), null);
  });
  ...
  it("fails closed when more than one release has the exact tag", () => {
    const tag = "zde-preview-v0.2.0-preview.3";
    assert.throws(
      () => resolveReleaseByTag([
        [{ id: 1, tag_name: tag }],
        [{ id: 2, tag_name: tag }],
  ...
  it("rejects malformed pages, entries, and tags", () => {
    for (const pages of [null, {}, [], [[]]]) {
      ...
      assert.throws(
        () => resolveReleaseByTag(pages, "zde-v1.0.0"),
        /array of API pages/,
      );
    }
    for (const entry of [null, [], "release", 7, {}]) {
      assert.throws(
        () => resolveReleaseByTag([[entry]], "zde-v1.0.0"),
        /non-object entry|missing tag_name/,
      );
    }
    for (const tag of [null, "", "zde-v1.0.0\n", 7]) {
      assert.throws(
