...
function normalizeArchivePath(path) {
  if (
    ...
    throw new Error("archive path must remain under ZDE.app/");
  }
...
function makeArchiveValidator(budgets) {
  ...
  return {
    ...
    result() {
      if (!seen.has("ZDE.app")) {
        throw new Error("archive must contain exactly one ZDE.app root");
      }
      if (entryTypes.get("ZDE.app") !== "directory") {
        throw new Error("archive ZDE.app root must be a directory");
      }
      return { ...state };
    },
  };
...
async function inspectExtractedTree(root, budgets) {
  ...
  if (basename(root) !== "ZDE.app") {
    throw new Error("extracted application root must be ZDE.app");
  }
  return result;
}
...
export async function extractSourceApplication(options) {
  ...
  try {
    ...
    const applicationPath = join(outputDirectory, "ZDE.app");
