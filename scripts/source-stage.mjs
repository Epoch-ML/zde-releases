...
function assertMetadata(metadata, request) {
  if (metadata?.schema_version !== 2 || metadata.product !== "ZDE") {
...
export async function prepareSourceStage(options) {
  ...
  const archiveName = `ZDE_${version}_aarch64.source.app.tar.gz`;
  ...
  if (basename(extraction.applicationPath) !== "ZDE.app") {
    throw new Error("source-stage archive did not extract one ZDE.app");
  }
